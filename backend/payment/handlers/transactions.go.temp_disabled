...
func (h *TransactionHandler) SendTransaction(c *gin.Context) {
	...
	if err := h.verifyAuthentication(c.Request.Context(), userUUID, req.Amount, req.Pin, req.Biometric); err != nil {
		responses.ErrorResponse(c, http.StatusForbidden, "Authentication required", err.Error())
		return
	}
	...
	if err != nil {
		responses.ErrorResponse(c, http.StatusBadRequest, "No default wallet", "No default wallet found for the specified currency.")
		return
	}
	...
	if err != nil {
		...
		case strings.Contains(err.Error(), "wallet not found"):
			responses.ErrorResponse(c, http.StatusNotFound, "Wallet not found", "The destination wallet does not exist.")
			return
		...
		case strings.Contains(err.Error(), "transaction limit"):
			responses.ErrorResponse(c, http.StatusBadRequest, "Transaction limit exceeded", "This transaction exceeds your daily/monthly limit.")
			return
		case strings.Contains(err.Error(), "wallet suspended"):
			responses.ErrorResponse(c, http.StatusBadRequest, "Wallet suspended", "One of the wallets is currently suspended.")
			return
		case strings.Contains(err.Error(), "same wallet"):
			responses.ErrorResponse(c, http.StatusBadRequest, "Invalid transaction", "Cannot transfer to the same wallet.")
			return
		default:
			responses.ErrorResponse(c, http.StatusInternalServerError, "Transaction failed", "Failed to process transaction.")
			return
		}
	}
...
func (h *TransactionHandler) TopUpWallet(c *gin.Context) {
	...
	if err != nil {
		...
		case strings.Contains(err.Error(), "unsupported gateway"):
			responses.ErrorResponse(c, http.StatusBadRequest, "Unsupported gateway", "The specified payment gateway is not supported.")
			return
		case strings.Contains(err.Error(), "amount too small"):
			responses.ErrorResponse(c, http.StatusBadRequest, "Amount too small", "The amount is below the minimum required for this gateway.")
			return
		case strings.Contains(err.Error(), "amount too large"):
			responses.ErrorResponse(c, http.StatusBadRequest, "Amount too large", "The amount exceeds the maximum allowed for this gateway.")
			return
		default:
			responses.ErrorResponse(c, http.StatusInternalServerError, "Payment creation failed", "Failed to create payment.")
			return
		}
	}
...
func (h *TransactionHandler) WithdrawFromWallet(c *gin.Context) {
	...
	if err := h.validateWithdrawalDestination(&req); err != nil {
		responses.ErrorResponse(c, http.StatusBadRequest, "Invalid withdrawal destination", err.Error())
		return
	}
	...
	if err != nil {
		...
		case strings.Contains(err.Error(), "withdrawal limit"):
			responses.ErrorResponse(c, http.StatusBadRequest, "Withdrawal limit exceeded", "This withdrawal exceeds your daily/monthly limit.")
			return
		case strings.Contains(err.Error(), "wallet suspended"):
			responses.ErrorResponse(c, http.StatusBadRequest, "Wallet suspended", "This wallet is currently suspended.")
			return
		default:
			responses.ErrorResponse(c, http.StatusInternalServerError, "Withdrawal failed", "Failed to process withdrawal.")
			return
		}
	}
...
func (h *TransactionHandler) GetTransactionHistory(c *gin.Context) {
	...
	if walletID == uuid.Nil {
		...
		if err != nil {
			responses.ErrorResponse(c, http.StatusInternalServerError, "Failed to get transaction history", "Unable to retrieve transaction history.")
			return
		}
...
func (h *TransactionHandler) GetTransaction(c *gin.Context) {
	...
	if err != nil {
		responses.ErrorResponse(c, http.StatusBadRequest, "Invalid transaction ID", "Transaction ID must be a valid UUID.")
		return
	}
	...
	if err != nil {
		if strings.Contains(err.Error(), "not found") {
			responses.ErrorResponse(c, http.StatusNotFound, "Transaction not found", "The specified transaction does not exist.")
			return
		}
		responses.ErrorResponse(c, http.StatusInternalServerError, "Failed to get transaction", "Unable to retrieve transaction.")
		return
	}
	...
	if err := h.verifyTransactionAccess(c.Request.Context(), transaction, userUUID); err != nil {
		responses.ErrorResponse(c, http.StatusForbidden, "Access denied", "You don't have access to this transaction.")
		return
	}
