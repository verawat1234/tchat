...
func NewRateLimiter(redisClient *redis.Client, cfg *config.Config, log *logger.Logger) *RateLimiter {
	return &RateLimiter{
		redis:  redisClient,
		config: cfg,
		logger: log,
...
func DefaultRateLimit(redisClient *redis.Client, cfg *config.Config, log *logger.Logger) gin.HandlerFunc {
...
func StrictRateLimit(redisClient *redis.Client, cfg *config.Config, log *logger.Logger) gin.HandlerFunc {
	rl := NewRateLimiter(redisClient, cfg, log)
	return rl.Limit(RateLimitConfig{
		Enabled:        true,
...
func GenerousRateLimit(redisClient *redis.Client, cfg *config.Config, log *logger.Logger) gin.HandlerFunc {
	rl := NewRateLimiter(redisClient, cfg, log)
	return rl.Limit(RateLimitConfig{
		Enabled:        true,
